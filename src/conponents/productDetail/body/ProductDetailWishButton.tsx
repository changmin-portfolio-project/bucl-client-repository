...
const WishButton: React.FC<HomeWishButtonComponentProps> = ({
  productCode,
  wished,
  style,
  svgStyle,
  ...
  return (
    <WishBtnContainer>
      <WishCheckBox type="checkbox" className="wish-checkbox" />
      {wished ? (
        <WishButtonLabel onClick={() => wishBtnOnClick(wished)} style={style}>
          <svg
            width="24"
            height="24"
            viewBox="0 0 24 24"
...
const WishCheckBox = styled.input`
  display: none;
`;
const WishButtonLabel = styled.label`
  display: flex;
